...
const DANGEROUS_VIDEO_URL = "https://cdn.sportingcampeao.pt/short.mp4";
const SLOWMO_VIDEO_URL = "https://cdn.sportingcampeao.pt/bolapequenov1.mp4";

export default function ShortPage() {
  const [showVideo, setShowVideo] = useState(false);
  const [showSlowmoVideo, setShowSlowmoVideo] = useState(false);
  const videoRef = useRef(null);
  const containerRef = useRef(null); // Ref para o container que entrará em fullscreen

  const handlePlayScare = async (isSlowmo = false) => {
    if (isSlowmo){
      setShowVideo(false);
      setShowSlowmoVideo(true)
    }
    else {
      setShowVideo(true);
      setShowSlowmoVideo(false)
    }
      ...
      {!showVideo && (
        <div className="text-center">
          ...
          <p className="text-lg text-muted-foreground mb-8 max-w-md mx-auto">
            Fez-me reviver os tempo de guarda-redes no carcavelinhos!
          </p>
          <Button
            onClick={handlePlayScare}
            size="lg"
            variant="destructive"
            className="bg-sporting hover:bg-sporting/90 text-white text-xl px-8 py-6"
          >
            <PlayCircle size={28} className="mr-3" />
            Iniciar
          </Button>
          <Button
            onClick={handlePlayScare(true)}
            size="lg"
            variant="destructive"
            className="bg-sporting hover:bg-sporting/90 text-white text-xl px-8 py-6"
          >
          ...
          <p className="mt-12 text-sm text-muted-foreground">
            <a href="/" className="hover:underline">← Voltar à Página Segura</a>
          </p>
        </div>
      )}

      {showVideo || showSlowmoVideo ? (
        // O vídeo ocupa o espaço do container que está em fullscreen
        <video
          ref={videoRef}
          src={showSlowmoVideo ? SLOWMO_VIDEO_URL : DANGEROUS_VIDEO_URL} // ** Use o seu URL aqui **
